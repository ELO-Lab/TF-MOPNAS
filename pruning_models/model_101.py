...
class Cell(nn.Module):
    ...
    def __init__(self, spec, in_channels, out_channels, bn=True):
        ...
        for t in range(1, self.num_vertices - 1):
            vertex_op_ = nn.ModuleList()
            for i_ in range(len(spec.ops[t])):
                op = OP_MAP[spec.ops[t][i_]](
                    self.vertex_channels[t], self.vertex_channels[t], bn=bn
                )
                vertex_op_.append(op)
            self.vertex_op.append(vertex_op_)
        ''' Ori '''
        # self.vertex_op = nn.ModuleList([None])
        # for t in range(1, self.num_vertices - 1):
    ...
    def forward(self, x):
        ...
        for t in range(1, self.num_vertices - 1):
            ...
            # vertex_output = self.vertex_op[t](vertex_input)  # ori
            ''' Custom '''
            out = []
            for i_ in range(len(self.spec.ops[t])):
                out.append(self.vertex_op[t][i_](vertex_input))
            vertex_output = sum(out)
            ''' --- '''
